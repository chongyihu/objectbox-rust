    ...
    fn drop(&mut self) {
        unsafe {
            if !self.obx_cursor.is_null() {
                self.error = c::call(
                    c::obx_cursor_close(self.obx_cursor),
                    "cursor::drop".to_string(),
                )
                .err();
